...
void print_line(tuple**print_t,int level,int size)
{
        ...
        else if (level>0)
        {
            ...
            for(i=1; i<(1<<level) ;i++)
            {
                ...
                else printf("Error while printing element in position %d\n",i);
            }
            free(numbers);
            free(elem);
            free(line);
...
void welcome(){
   printf("\nWelcome to the Red-Black Tree Builder!\n"
           "I'll handle all keys for you!\n"
           "Insert a value : \n");
           return;
}
...
void case1(mynode*node,int ruota)
{
    ...
    else printf("Case1 generic error\n");
}
    ...
    else 
    {
        if(val <= node->value ) {
            if(node->left==NULL){
                ...
                printf("Value %d is lower than : %d, going left\n",val,node->value);
            ...
            if(node->right==NULL)
            {
                printf("Inserting value %d on the right of node : %d\n",val,node->value);
                node->right=red_node_create(node,key,val);
                tree_print(root);
                fix(node->right,root);
            }
            else {
                printf("Value %d is grater than : %d, going right\n",val,node->value);
